// ============================================================================
// AI-901 Prep Hub — Motor da Aplicação (js/app.js)
...
const GENERAL_ACHIEVEMENTS = [
  ...
  { id: "todos_dominios", name: "Panorama Completo", icon: "🗺️", desc: "Acerte ao menos uma questão de cada um dos 10 domínios do edital.",
    check: p => p.stats.domainsSeen.length >= 10 },
  ...
  { id: "especialista_ia", name: "Especialista em IA", icon: "🌟", desc: "Alcance o nível 10.",
    check: p => levelInfo(p.xp).level >= 10 },
  { id: "praticante", name: "Praticante", icon: "🧪", desc: "Complete seu primeiro laboratório prático no Microsoft Foundry.",
  ...
  { id: "arquiteto_ia", name: "Arquiteto de IA", icon: "☁️", desc: "Complete todos os 10 laboratórios práticos.",
    check: p => (p.stats.labsCompleted || []).length >= LAB_BANK.length },
];

// Uma conquista "Expert em <domínio>" para cada um dos 10 domínios do edital,
...
RENDERERS.bank = function renderBank() {
  ...
  el.innerHTML = `
    ...
    <p class="lead">200 questões originais, no estilo Microsoft Learn, cobrindo todos os domínios da AI-901. Pratique com feedback detalhado a cada resposta.</p>
...
RENDERERS.games = function renderGamesMenu() {
  ...
  el.innerHTML = `
    ...
    <p class="lead">Aprenda conceitos da AI-901 de forma divertida e ganhe XP.</p>
...
// ----------------------------------------------------------------------------
// 13) LABORATÓRIOS PRÁTICOS (portal real do Microsoft Foundry)
// ----------------------------------------------------------------------------
RENDERERS.labs = function renderLabsMenu() {
  ...
  el.innerHTML = `
    ...
    <p class="lead">Coloque a mão na massa no portal real do Microsoft Foundry (ai.azure.com). Cada laboratório traz um passo a passo guiado e, ao final, um pequeno teste de verificação — só quem realmente navegou pelas telas responde com facilidade.</p>
    <div class="labs-warning">⚠️ Use um projeto Foundry gratuito ou de testes. Algumas implantações de modelo podem gerar custo — remova implantações que não for mais utilizar ao final da sessão.</div>
  ...
  area.innerHTML = `
    <div class="question-card lab-detail-card">
      ...
      <div class="game-actions">
        <a class="btn btn-primary" href="https://ai.azure.com" target="_blank" rel="noopener">🔗 Abrir Microsoft Foundry</a>
