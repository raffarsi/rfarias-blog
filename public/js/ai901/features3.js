...
const INTERACTIVE_LABS = (function() {
  ...
  const dp900Labs = [
    {
      ...
      intro: 'Neste lab você vai praticar a escolha do serviço de dados correto para cada cenário real.',
      steps: [
        {
          instruction: 'Um e-commerce precisa armazenar pedidos com tabelas de clientes, produtos e itens relacionados, com transações ACID. Qual serviço?',
          type: 'choice',
          options: ['Azure Cosmos DB', 'Azure Blob Storage', 'Azure SQL Database', 'Azure Data Lake'],
          correct: 2,
          feedback: 'Correto! Azure SQL Database é ideal para dados relacionais com tabelas e relacionamentos. Pedidos, produtos e clientes precisam de joins, foreign keys e transações ACID.',
        },
        {
          instruction: 'Uma rede social global precisa armazenar perfis com campos diferentes por usuário e latência abaixo de 10ms em qualquer país. Qual serviço?',
          type: 'choice',
          options: ['Azure SQL Database', 'Azure Cosmos DB', 'Azure Table Storage', 'Azure Files'],
          correct: 1,
          feedback: 'Correto! Azure Cosmos DB foi projetado para distribuição global com latência garantida abaixo de 10ms e esquema flexível. Perfeito para perfis de usuários variados em escala global.',
        },
        {
          instruction: 'Um analista precisa rodar queries complexas sobre 5 anos de dados de vendas (500 GB) para relatórios mensais. Qual serviço?',
          type: 'choice',
          options: ['Azure SQL Database', 'Azure Cosmos DB', 'Azure Synapse Analytics', 'Azure Cache for Redis'],
          correct: 2,
          feedback: 'Correto! Azure Synapse Analytics é otimizado para OLAP — queries analíticas sobre grandes volumes históricos. Azure SQL Database é OLTP — para transações do dia a dia.',
        },
        {
          instruction: 'Uma aplicação web precisa de sessões de usuário com leitura ultra-rápida (sub-milissegundo). Os dados podem ser perdidos se o servidor reiniciar. Qual serviço?',
          type: 'choice',
          options: ['Azure SQL Database', 'Azure Cosmos DB', 'Azure Cache for Redis', 'Azure Table Storage'],
          correct: 2,
          feedback: 'Correto! Azure Cache for Redis é um banco em memória com latência sub-milissegundo, ideal para sessões e cache. Por ser em memória, dados voláteis podem ser perdidos em reinicializações — aceitável para sessões.',
        },
      ],
    },
    {
      id: 'ilab-dp900-02',
      title: 'Construir um pipeline de dados no Azure Data Factory',
      domain: 'Cargas de Trabalho de Analytics no Azure',
      intro: 'Neste lab você vai tomar decisões corretas para construir um pipeline ETL no Azure Data Factory.',
      steps: [
        {
          instruction: 'Você precisa copiar dados de um SQL Server on-premises para o Azure Data Lake diariamente às 2h. Qual componente do ADF configura isso?',
          type: 'choice',
          options: ['Dataset', 'Linked Service', 'Trigger de Agendamento (Schedule Trigger)', 'Integration Runtime'],
          correct: 2,
          feedback: 'Correto! Triggers controlam quando pipelines são executados. O Schedule Trigger permite definir horários específicos. O Integration Runtime é o motor de execução; o Linked Service é a conexão com a fonte.',
        },
        {
          instruction: 'Para conectar o ADF ao SQL Server dentro da rede da empresa (sem IP público), qual componente é necessário?',
          type: 'choice',
          options: ['Azure Integration Runtime (padrão)', 'Self-hosted Integration Runtime', 'Linked Service apenas', 'VPN Gateway'],
          correct: 1,
          feedback: 'Correto! O Self-hosted Integration Runtime é um agente instalado dentro da rede da empresa que permite ao ADF acessar fontes de dados internas sem expô-las à internet.',
        },
        {
          instruction: 'Qual a diferença fundamental entre ETL e ELT no contexto de data lakes modernos?',
          type: 'choice',
          options: [
            'ETL transforma antes de carregar; ELT carrega dados brutos primeiro e transforma no destino',
            'ETL é mais rápido; ELT é mais seguro',
            'ETL usa SQL; ELT usa Python',
            'Não há diferença',
          ],
          correct: 0,
          feedback: 'Correto! ETL transforma os dados antes de carregar (padrão antigo). ELT carrega dados brutos no Data Lake e transforma lá com Synapse ou Databricks. ELT é o padrão moderno pois preserva os dados originais.',
        },
      ],
    },
    {
      id: 'ilab-dp900-03',
      title: 'Consultar dados no Azure Synapse Analytics',
      domain: 'Cargas de Trabalho de Analytics no Azure',
      intro: 'Neste lab você vai tomar decisões sobre como usar o Azure Synapse Analytics para consultar e analisar dados em escala.',
      steps: [
        {
          instruction: 'Você tem arquivos Parquet no Data Lake e quer fazer uma query exploratória rápida sem provisionar recursos. Qual recurso do Synapse usar?',
          type: 'choice',
          options: ['Dedicated SQL Pool', 'Synapse Spark Pool', 'Serverless SQL Pool', 'Azure SQL Database'],
          correct: 2,
          feedback: 'Correto! O Serverless SQL Pool permite consultar arquivos no Data Lake com SQL sem provisionar recursos dedicados. Você paga apenas pelas queries executadas — ideal para exploração e análises ad-hoc.',
        },
        {
          instruction: 'Sua equipe precisa de dashboards que atualizam em menos de 2 segundos com queries complexas sobre 1 TB de dados. Qual recurso?',
          type: 'choice',
          options: ['Serverless SQL Pool', 'Dedicated SQL Pool com distribuição hash', 'Azure SQL Database', 'Azure Cosmos DB'],
          correct: 1,
          feedback: 'Correto! O Dedicated SQL Pool provê recursos computacionais dedicados com distribuição dos dados entre nós para máxima performance. Ideal para relatórios com SLA de performance garantida.',
        },
        {
          instruction: 'Qual é a principal diferença entre dados OLTP e OLAP?',
          type: 'choice',
          options: [
            'OLTP é para bancos NoSQL; OLAP é para bancos relacionais',
            'OLTP é otimizado para transações do dia a dia (insert/update rápidos); OLAP é otimizado para queries analíticas sobre grandes volumes históricos',
            'OLTP é mais caro que OLAP',
            'Não há diferença prática entre eles',
          ],
          correct: 1,
          feedback: 'Correto! OLTP (Online Transaction Processing): muitas transações pequenas e rápidas — sistema de vendas, ERP. OLAP (Online Analytical Processing): poucas queries complexas sobre grandes volumes — relatórios, BI. Azure SQL Database é OLTP; Synapse é OLAP.',
        },
      ],
    },
  ];
  ...
  const ai901Labs = [
    {
      id: 'ilab-ai901-01',
      title: 'Escolher o serviço de IA certo para cada cenário',
      domain: 'Panorama de Workloads de IA',
      intro: 'Azure tem dezenas de serviços de IA. Neste lab você vai praticar a escolha certa para cada situação real.',
      steps: [
        {
          instruction: 'Um e-commerce quer analisar automaticamente o sentimento de milhares de avaliações de produtos. Qual serviço Azure usar?',
          type: 'choice',
          options: ['Azure Computer Vision', 'Azure Language Service (Análise de Sentimento)', 'Azure Bot Service', 'Azure Speech Service'],
          correct: 1,
          feedback: 'Correto! O Azure Language Service tem uma API específica de Análise de Sentimento que classifica texto como positivo, negativo ou neutro. Ideal para processar avaliações em escala.',
        },
        {
          instruction: 'Uma empresa quer extrair automaticamente dados de formulários físicos digitalizados (nome, CPF, data) sem treinar um modelo do zero. Qual serviço?',
          type: 'choice',
          options: ['Azure Custom Vision', 'Azure Machine Learning', 'Azure Document Intelligence (Form Recognizer)', 'Azure Cognitive Search'],
          correct: 2,
          feedback: 'Correto! O Azure Document Intelligence tem modelos pré-treinados para formulários, recibos e documentos. Extrai campos estruturados sem necessidade de treinamento.',
        },
        {
          instruction: 'Um assistente de IA médica toma decisões de diagnóstico sem nenhuma explicação de como chegou àquela conclusão. Qual princípio de IA Responsável está sendo violado?',
          type: 'choice',
          options: ['Inclusão', 'Responsabilização', 'Transparência', 'Equidade'],
          correct: 2,
          feedback: 'Correto! O princípio de Transparência exige que sistemas de IA sejam explicáveis. Em medicina, entender como o diagnóstico foi gerado é crítico para validar e responsabilizar decisões.',
        },
        {
          instruction: 'Qual parâmetro do Azure OpenAI controla a criatividade e aleatoriedade das respostas geradas?',
          type: 'choice',
          options: ['max_tokens', 'temperature', 'top_p apenas', 'frequency_penalty'],
          correct: 1,
          feedback: 'Correto! Temperature controla a aleatoriedade: valores próximos de 0 geram respostas determinísticas e conservadoras; valores próximos de 1 geram respostas mais criativas e variadas.',
        },
        {
          instruction: 'Sua empresa quer usar GPT-4o garantindo que dados não saiam do Brasil (conformidade com LGPD). Qual abordagem?',
          type: 'choice',
          options: ['Usar API da OpenAI diretamente — dados são criptografados', 'Azure OpenAI com recurso na região Brazil South', 'Azure OpenAI em East US — mais rápido e igualmente seguro', 'Não é possível usar GPT-4o em conformidade com LGPD'],
          correct: 1,
          feedback: 'Correto! O Azure OpenAI mantém dados dentro da infraestrutura Azure por região. Brazil South garante residência de dados no país. A API da OpenAI direta não oferece essa garantia contratual.',
        },
      ],
    },
    {
      id: 'ilab-ai901-02',
      title: 'Aplicar os princípios de IA Responsável',
      domain: 'IA Responsável',
      intro: 'A Microsoft define 6 princípios de IA Responsável. Neste lab você vai identificar qual está sendo violado ou aplicado em cada cenário.',
      steps: [
        {
          instruction: 'Um sistema de concessão de crédito usa IA e aprova menos empréstimos para pessoas de determinados bairros. Qual princípio está sendo violado?',
          type: 'choice',
          options: ['Confiabilidade e Segurança', 'Equidade (Fairness)', 'Privacidade e Segurança', 'Inclusão'],
          correct: 1,
          feedback: 'Correto! O princípio de Equidade exige que sistemas de IA não discriminem grupos. O modelo aprendeu padrões discriminatórios dos dados históricos — o que exige auditoria e rebalanceamento dos dados de treino.',
        },
        {
          instruction: 'Uma empresa usa reconhecimento facial de funcionários sem informá-los ou obter consentimento. Qual princípio está sendo violado?',
          type: 'choice',
          options: ['Privacidade e Segurança', 'Confiabilidade', 'Inclusão', 'Equidade'],
          correct: 0,
          feedback: 'Correto! O princípio de Privacidade e Segurança exige que dados biométricos como rostos sejam coletados com consentimento explícito e protegidos contra acesso não autorizado.',
        },
        {
          instruction: 'Após um acidente causado por um carro autônomo, a empresa diz "o algoritmo decidiu, não nós". Qual princípio está sendo violado?',
          type: 'choice',
          options: ['Equidade', 'Responsabilização (Accountability)', 'Transparência', 'Confiabilidade'],
          correct: 1,
          feedback: 'Correto! O princípio de Responsabilização estabelece que pessoas e organizações são responsáveis pelos sistemas de IA que criam e implantam. "O algoritmo decidiu" não é uma defesa válida.',
        },
      ],
    },
    {
      id: 'ilab-ai901-03',
      title: 'Configurar Azure OpenAI no portal',
      domain: 'Modelos de IA Generativa e Configuração',
      intro: 'Neste lab você vai tomar as decisões corretas para configurar e usar o Azure OpenAI Service em um projeto corporativo.',
      steps: [
        {
          instruction: 'O modelo está inventando informações sobre seus produtos que não existem ("alucinando"). Qual a melhor solução?',
          type: 'choice',
          options: ['Aumentar max_tokens para o modelo ter mais espaço', 'Implementar RAG — fornecer documentos reais como contexto ao modelo', 'Diminuir a temperature para 0', 'Trocar para um modelo menor'],
          correct: 1,
          feedback: 'Correto! RAG (Retrieval-Augmented Generation) é a solução padrão para alucinação. Em vez de depender do conhecimento do modelo, você fornece documentos reais como contexto. O modelo responde com base nesses dados.',
        },
        {
          instruction: 'Você precisa que o modelo sempre responda em português formal e nunca mencione concorrentes. Onde configurar isso?',
          type: 'choice',
          options: ['No parâmetro "user" da API', 'No System Prompt (mensagem de sistema)', 'No parâmetro temperature', 'Não é possível configurar esse comportamento'],
          correct: 1,
          feedback: 'Correto! O System Prompt define o comportamento base do modelo — idioma, tom, restrições e persona. É a primeira mensagem do tipo "system" na conversa e tem prioridade sobre as instruções do usuário.',
        },
        {
          instruction: 'Qual é a diferença entre o Azure OpenAI Service e a API da OpenAI diretamente?',
          type: 'choice',
          options: [
            'São idênticos — mesmos modelos, mesmos dados, mesmos preços',
            'Azure OpenAI: dados ficam na sua assinatura Azure, SLA empresarial, conformidade regulatória. OpenAI API: dados processados pela OpenAI com suas políticas',
            'Azure OpenAI é mais lento mas mais barato',
            'Azure OpenAI só suporta GPT-3.5, não GPT-4',
          ],
          correct: 1,
          feedback: 'Correto! A diferença principal é governança de dados e conformidade. O Azure OpenAI processa dados dentro da sua assinatura Azure com SLA empresarial, GDPR, LGPD e ISO 27001. Ideal para dados corporativos sensíveis.',
        },
      ],
    },
  ];
  ...
  if (url.includes('dp900')) return [];
  if (url.includes('ai901')) return ai901Labs;
  if (url.includes('ms-project')) return [];
