...
É tentador assumir que a geração do modelo domina o tempo total, afinal, o modelo pode levar segundos dependendo do tamanho da resposta, e ignorar a rede como fator secundário. Na prática, quando a arquitetura está mal desenhada em termos de região e conectividade, a rede pode adicionar centenas de milissegundos em saltos que deveriam ser desprezíveis.

O problema não é que a rede vai dominar o tempo total, provavelmente não vai. O problema é que latência de rede mal desenhada é **variável e imprevisível**, e soma variação a um componente que já varia: em deployments Standard e Global Standard, o próprio tempo do modelo oscila com a carga do serviço, e só o provisionado (PTU) tem latência previsível. Picos dos dois lados transformam um P99 aceitável num P99 inaceitável para usuários.
...
Usuário → Agente (Foundry) → Azure OpenAI (embedding) → [vetor da pergunta]
                           ...
                           → Azure OpenAI (geração)   → [resposta gerada]
                           → Usuário
```
...
**Salto 2:** Agente → Azure OpenAI, para gerar o embedding da pergunta com o modelo de embedding. (Se o índice usa vetorização integrada, quem faz essa chamada é o próprio AI Search.)

**Salto 3:** Agente → Azure AI Search. A busca híbrida com o vetor da pergunta. Deve ser intra-região ou via Private Link para minimizar latência.

**Salto 4:** Agente → Azure OpenAI, a chamada de geração, com a resposta voltando em streaming para o usuário. Latência dominada pelo tempo de inferência do modelo, mas com overhead de rede significativo se os recursos estiverem em regiões diferentes.

Os saltos internos, Agente ↔ OpenAI e Agente ↔ Search, são os que a arquitetura de rede pode e deve otimizar. Os saltos externos dependem da localização do usuário.

## Fator 1: região dos recursos

O erro mais comum e mais caro: o agente em uma região, Azure AI Search em outra, Azure OpenAI em uma terceira.
...
**Para o Brasil:** Brazil South tem disponibilidade limitada de modelos no deployment regional, e regiões populares nos Estados Unidos, como East US, costumam ter capacidade disputada. Duas saídas comuns: deployments Global Standard ou Data Zone Standard, que distribuem o processamento e têm mais capacidade (se o requisito de residência de dados permitir), ou agente, Search e modelo juntos numa mesma região fora do Brasil. Nesse segundo caso, o Azure Front Door ajuda a encurtar o caminho do usuário até lá, porque recebe a conexão na borda mais próxima e segue pela rede da Microsoft. O Traffic Manager não serve para isso: ele só responde DNS e não carrega o tráfego.
...
# Sem Semantic Ranker (busca só por texto, BM25)
...
print(f"BM25: {t_bm25*1000:.0f}ms")
...
Envie esses dados para o Application Insights e monitore o P50, P90 e P99 de cada componente separadamente. A distribuição vai mostrar onde está a variabilidade, e se ela vem da rede ou do modelo.
